...
class Managebill extends CI_Controller {
	...
	<a href="'.site_url("managebill/printCashBill/$1").'" class="btn btn-success btn-xs" target="_blank" data-title="View" data-toggle="tooltip" data-target="#view" data-placement="top" rel="tooltip" title="ดูรายละเอียด"><span class="glyphicon glyphicon-print"></span></a>
    <a href="'.site_url("managebill/editbill/$1").'" class="btn btn-primary btn-xs" data-title="Edit" data-toggle="tooltip" data-target="#edit" data-placement="top" rel="tooltip" title="แก้ไข"><span class="glyphicon glyphicon-pencil"></span></a>
	<button class="btnDelete btn btn-danger btn-xs" onclick="del_confirm($1)" data-title="Delete" data-toggle="modal" data-target="#delete" data-placement="top" rel="tooltip" title="ลบข้อมูล"><span class="glyphicon glyphicon-remove"></span></button> </div> ',"id");
    ...
    function deletebill()
    {
		...
		$result = $this->bill->delBill($id);
		redirect('managebill/viewBillByBranch/'.$branchid, 'refresh');
    }
    ...
    function editbill()
    {
		...
		$query = $this->bill->getOneBillProduct($id);
		if($query){
			$data['billproduct_array'] =  $query;
		}else{
			$data['billproduct_array'] = array();
        ...
        $data['id'] = $id;
		$data['title'] = "Pradit and Friends - Edit Bill";
		$this->load->view("editproduct_bill_view", $data);

    }

    function deleteproduct_bill()
	{
		$id = $this->uri->segment(3);
        $billid = $this->uri->segment(4);
		$result = $this->bill->delProductBill($id);
		redirect('managebill/editbill/'.$billid, 'refresh');
	}

    function editproduct_amount_bill()
	{
		...
		$query = $this->bill->editAmountProductBill($editbill);

	}

    function addproduct_editbill()
    {
        $id=$this->input->post('id');
        $barcodeid= ($this->input->post('barcode'));
    ...
    function previewCashBill_edit()
    {
		$data['showresult'] = 'success';
		$data['billid'] = $this->input->post('editid');
