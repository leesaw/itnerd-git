...
class Managestock extends CI_Controller {
    ...
    function historystockexcel_in()
    {
        ...
        if ($productid!="") {
            $sql .= " where standardID = '".$productid."'";
        }
    ...
    function excelcarnumber_out_count()
	{	
        $carnumber = $this->input->post("carnumber");
        $start = $this->input->post("startdate");
		if ($start != "") {
        ...
        $delimiter = ",";
        $newline = "\r\n";
		$sql = "select concat('\'',standardID,'\'') as รหัสสินค้า, product.name as ชื่อสินค้า, sum(amount) as จำนวน, unit as หน่วย,priceVAT as ราคาขาย, sum(amount)*priceVAT as จำนวนเงิน";
		$sql .= " from stock_out";
		$sql .= " left join product on product.id = stock_out.productID";
		$sql .= " left join branch on branch.id = stock_out.branchID";
		$sql .= " left join category on category.id = product.categoryID";
