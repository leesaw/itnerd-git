...
<html>
<head>
<title>Stock Out Printing</title>
</head>
<body>
<table border="0">
<tbody>
...
<td width="50"> </td>
<td width="200"><div style="text-align: right; font-weight: bold; font-size: 16pt;">ใบรายการเอาสินค้าออกจากคลัง</div></td>
</tr>
<?php foreach($stock_array as $loop) { $datetime = $loop->stoo_datein; $si_id = $loop->stoo_number; $editor = $loop->firstname." ".$loop->lastname; $stock_name = $loop->wh_code."-".$loop->wh_name; $dateadd = $loop->stoo_dateadd; $remark = $loop->stoo_remark; break; }
...
?>
<tr>
    <td>คลังที่เอาออก : <?php echo $stock_name; ?><br>ชื่อผู้ใส่ข้อมูล:  <?php echo $editor; ?><br>Remark: <?php echo $remark; ?></td><td> </td><td>เลขที่ <?php echo $si_id; ?><br> วันที่ : <?php echo $GGdate."/".$GGmonth."/".$GGyear; ?><br>วันที่บันทึก : <?php echo $dateadd; ?>
    </td>
</tr>
</tbody>
</table>
...
<thead>
	<tr>
		<th width="30" style="border-bottom:1px solid black;">No.</th><th width="280" style="border-left:1px solid black;border-bottom:1px solid black;">Ref. Number/รายละเอียดสินค้า</th><th width="80" style="border-left:1px solid black;border-bottom:1px solid black;">จำนวน<br>ในคลัง</th><th width="80" style="border-left:1px solid black;border-bottom:1px solid black;">จำนวน<br>เอาออก</th><th width="80" style="border-left:1px solid black;border-bottom:1px solid black;">จำนวน<br>ในคลังหลังเอาออก</th><th width="100" style="border-left:1px solid black;border-bottom:1px solid black;">หน่วยละ</th><th width="120" style="border-left:1px solid black;border-bottom:1px solid black;">จำนวนเงิน</th>
	</tr>
</thead>
<tbody>
...
if(isset($serial_array)) {
    foreach ($serial_array as $loop2) {
        if (($loop->log_stoo_item_id==$loop2->log_stoo_item_id)) { ?>
...
<tbody>
<tr><td width="350" align="center">ผู้นำสินค้าออกจากคลัง</td><td width="350" align="center">ผู้อนุมัติ</td>
</tr>
<tr><td> </td><td>&nbsp;</td></tr>
