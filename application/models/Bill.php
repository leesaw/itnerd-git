...
Class Bill extends CI_Model
{
 ...
 {
	$this->db->select("bill.id as bid, billID, date, customer.customerID as _customerID, customerName, customerAddress, customerTel, customerFax, customerContact, bill.discount as bdiscount, bill.discountpercent as bdiscount2 , tax, title, users.firstname as fname, users.lastname as lname, bill.creditDay as bcreditDay, bill.status as bstatus, transport, percentvat, branch.name as branchname, bill.saleprice as _saleprice");
	$this->db->from('bill');	
	$this->db->join('customer','customer.id=bill.customerID','left');
	$this->db->join('users','users.id=bill.userID');
    $this->db->join('branch','branch.id=bill.branchID');
 ...
 {
	$this->db->_protect_identifiers=false;
	$this->db->select("bill_product.productID as pid, amount, pricePerUnit, CONCAT(product.standardID,' ', product.name) as productname, unit, priceNoVAT, priceDiscount, tax, discount, discountPercent, product.barcode as _barcode, lowestPrice, priceVAT, bill_product.id as _id ");
 ...
 function delBill($id=NULL)
 {
	$this->db->where('id', $id);
	$this->db->delete('bill'); 
     
    $this->db->where('billID', $id);
	$this->db->delete('bill_product'); 
 }
    
 function delProductBill($id=NULL)
 {     
    $this->db->where('id', $id);
	$this->db->delete('bill_product'); 
 }
 ...
 function editPriceTemp($temp=NULL,$userid=null,$status=null)
 {
  ...
  function editAmountProductBill($temp=NULL)
 {
	$this->db->where('id', $temp['id']);
	unset($temp['id']);
	$query = $this->db->update('bill_product', $temp); 	
	return $query;
 }
